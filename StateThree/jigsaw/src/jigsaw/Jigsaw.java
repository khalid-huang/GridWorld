...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		while(!this.openList.isEmpty() && searchedNodesNum < maxNodesNum) {
			...
			searchedNodesNum++; //表示已经访问

			pw.println("Searhing.....Number of seached nodes:" + this.closeList.size() + " Current sate:" + this.currentJNode.toString());
			System.out.println("Searhing......Number of seached nodes: " + this.closeList.size() + " Current state" + this.currentJNode.toString());

			Vector<JigsawNode> followJNodes = findFollowJNodes(this.currentJNode);
	...
	private void estimateValue(JigsawNode jNode) {
		int weightOne = 3;
		int weightDistance = 5;
		int weightWrongNode = 0;
		int estimate = weightOne * estimateOne(jNode) + weightDistance * estimateTwo(jNode) + weightWrongNode * estimateThree(jNode);
		jNode.setEstimatedValue(estimate);
	}
	
	//后续节点不正确的数码个数
	private int estimateOne(JigsawNode jNode) 
	{
		int s = 0;
	...
	//放错位的数码与其正确位置的黎曼距离之和(权重较大)
	private int estimateTwo(JigsawNode jNode) 
	{
		int s = 0;
		int dimension = JigsawNode.getDimension();
		int[] curState = jNode.getNodesState();
		int disRow, disCol;
		for(int index = 1; index <= dimension * dimension; index++)
		{
			if(curState[index] != index && curState[index] != 0)
			{
				disRow = Math.abs((curState[index] - 1) / dimension - (index - 1) / dimension);
				disCol = Math.abs((curState[index] -1) % dimension - (index - 1) % dimension);
				s += disCol + disRow;
			}
		}
		return s;
	...
	//放错位的数码个数
	private int estimateThree(JigsawNode jNode)
	{
		int s = 0;
		int dimension = JigsawNode.getDimension();
		for(int index = 1; index < dimension * dimension; index++)
		{
			if(index != jNode.getNodesState()[index] && jNode.getNodesState()[index] != 0)
			{
				s++;
			}
		}
